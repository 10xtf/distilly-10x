...
The test is whether an implementer can act on the row alone. "This mechanism is standard practice" fails it; "one row per subject, so a worker that falls behind upserts to the latest state and dedupes for free" passes, because the reason names the failure it avoids. Both design parents follow this rule, including the deprecated [system-v1.md](../../../../docs/design/system-v1.md), which is held to the same standard as [system-v2.md](../../../../docs/design/system-v2.md) because it is equally public.
