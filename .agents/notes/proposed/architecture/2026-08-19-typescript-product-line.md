...
There is still no runnable user product or `~/.distilly/` installation. The current tree contains the workspace, the built `@distilly/protocol` types and runtime schemas, and `@distilly/engine` through its package-internal Step 7 slice: verified facts and locks; atomic create/existing text ingest; deterministic source grouping and briefing; durable lease transactions; deterministic evidence resolution, claim application, quality, review-gate, and rendering functions; fully verified immutable version publication; current-or-suspended state commit; and target-first crash recovery over the disposable SQLite queue projection. Step 8 adds the browser-safe injected-client `distilly` package plus the transport-neutral exact-five `@distilly/mcp` adapter and separate stdio entry. Step 9 adds capability-only `@distilly/bindings` contracts/registry and Codex/Claude Code preflight translators, plus one recursive canonical skill tree, exact host mirrors, platform manifests, and a byte-verified release assembler. The Engine root still exports no partial API, the stdio child injects a full test fake, and the host capability entries do not install or construct a runtime. A complete `EngineRuntime`, full binding, production MCP/CLI/setup composition, review/correction/evolution services, and Panel remain unimplemented. Changing the product language therefore still does not migrate running user data or behavior.
...
- **`distilly` is a facade package** whose current browser-safe root holds `Distilly` and `Person` over an injected `EngineClient`; the future Node composition entry owns `openInProcess` only after a complete runtime exists. Swapping an eventual in-process engine for a daemon changes that composition entry rather than the public handles. A separate thin client package would have had exactly one consumer, so it is not cut.
