...
There is still no runnable user product or `~/.distilly/` installation. The current tree contains the workspace, the built `@distilly/protocol` types and runtime schemas, and `@distilly/engine` through its package-internal Step 5 slice: verified facts and locks, atomic create/existing text ingest, root idempotency and prepared-journal recovery, pending-state derivation, and the disposable SQLite queue projection with fact-driven rebuild. The package root deliberately exports no partial Engine API; complete briefing/commit/runtime composition, the facade, MCP server, CLI, bindings, and Panel remain unimplemented. Changing the product language therefore still does not migrate running user data or behavior.
