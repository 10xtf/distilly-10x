...
There is still no runnable user product or `~/.distilly/` installation. The current tree contains the workspace, the built `@distilly/protocol` types and runtime schemas, and `@distilly/engine` through its package-internal Step 6 slice: verified facts and locks; atomic create/existing text ingest; deterministic source grouping; verified version/claims baseline reads; packaged host-distill prompt loading; fail-closed briefing capacity; state-authoritative brief, renew, and release lease transactions with crash recovery; and the disposable SQLite queue v2 projection with fact-driven rebuild. The package root deliberately exports no partial Engine API; a complete `EngineRuntime`, commit/version writer, facade, MCP runtime, CLI, bindings, and Panel remain unimplemented. Changing the product language therefore still does not migrate running user data or behavior.
