...
There is still no runnable product or user `~/.distilly/` installation. The current tree contains the workspace, the built `@distilly/protocol` types and runtime schemas, and `@distilly/engine`'s internal fact foundation: layout, canonical hashes, atomic local files, concrete stores, and writer locks. The complete Engine runtime, create/ingest and recovery services, queue, facade, MCP server, CLI, bindings, and Panel remain unimplemented. Changing the product language therefore still does not migrate running user data or behavior.
