...
There is still no runnable user product or `~/.distilly/` installation. The current tree contains the workspace, the built `@distilly/protocol` types and runtime schemas, and `@distilly/engine` through its package-internal Step 7 slice: verified facts and locks; atomic create/existing text ingest; deterministic source grouping and briefing; durable lease transactions; deterministic evidence resolution, claim application, quality, review-gate, and rendering functions; fully verified immutable version publication; current-or-suspended state commit; and target-first crash recovery over the disposable SQLite queue projection. Local Step 8 adds the browser-safe `distilly` package with `Distilly` and `Person` over a supplied complete `EngineClient`, plus the transport-neutral exact-five `@distilly/mcp` adapter and its separate stdio entry over supplied client/presenter ports. The Engine root still exports no partial API, and the real stdio child injects a full test fake rather than a `DISTILLY_ROOT` backend. A complete `EngineRuntime`, production MCP/CLI/setup composition, review/correction/evolution services, bindings, and Panel remain unimplemented. Changing the product language therefore still does not migrate running user data or behavior.
