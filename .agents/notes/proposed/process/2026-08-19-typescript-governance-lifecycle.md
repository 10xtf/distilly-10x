...
The implemented slice is a pinned `pnpm` workspace, the V3 §29.1 `@distilly/protocol` contract, and `@distilly/engine` through its internal Step 5 atomic-ingest/recovery/queue slice. Its real stages cover formatting and lint, no-emit typecheck, fail-closed Vitest discovery, reported coverage, readable Protocol snapshots, TypeScript build, package self-import, exports and type resolution, and unused or undeclared dependency hygiene. Protocol gates cover the 35-method schema registry, the exact five MCP descriptors, an explicit public-export allowlist, and the built entry. The second package adds a fail-closed TypeScript-AST gate for manifest/source dependency direction, relative-import bypasses, non-static module targets, and package cycles; it follows `npm:` and `workspace:` aliases plus local `link:`, directory `file:`, and bare directory specs back to the real internal target instead of trusting the dependency key. Engine fixtures exercise real facts, cross-process locks, atomic create/existing ingest, global RequestId replay and conflict, prepared-journal abort/target-first recovery across crash points, and fact-driven SQLite queue rebuild, while its root stays empty until a complete runtime exists. The built smoke imports `@distilly/engine` by package-name self-reference so the export map is part of the check. The CI definition exercises Node 22.19 and 24 across Linux and macOS; the Python documentation, Agent Note, compile, unittest, and Ruff lanes remain in place for the legacy tree. The governed-diff predicate includes `packages/` and the root pnpm, TypeScript, ESLint, Vitest, Knip, and Prettier configuration surfaces while preserving the standing tests-only exemption for co-located Vitest files and snapshots.

This evidence proves the workspace, serializable Protocol contract, package boundary, Engine fact primitives, and the internal Step 5 ingest/recovery/queue behavior, not the full lifecycle proposed here. Model-visible `prompt()` / `SKILL.md` / host-instruction snapshots, the complete public EngineRuntime and later method pipeline, host workflows, fresh install, release automation, and eventual parity-backed retirement of Python governance remain unimplemented, so this Note remains proposed.
