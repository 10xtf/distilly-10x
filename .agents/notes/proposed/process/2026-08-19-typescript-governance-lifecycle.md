...
The implemented local slice is a pinned `pnpm` workspace, the V3 §29.1 `@distilly/protocol` contract, `@distilly/engine` through its package-internal Step 7 claim-commit slice, and the Step 8 injected-client `distilly` Facade plus `@distilly/mcp` adapter and stdio transport. Its real stages cover formatting and lint, no-emit typecheck across all four packages, fail-closed Vitest discovery, reported coverage, reviewed Protocol/prompt/public-export checks, TypeScript build, package self-import, exports and type resolution, and unused or undeclared dependency hygiene. Protocol gates cover the 35-method schema registry, exact five MCP descriptors, loopback ReviewLaunch relation, wire failures including redacted `internal_error`, an explicit public-export allowlist, and the built entry. Engine fixtures exercise real facts, cross-process locks, atomic create/existing ingest, global RequestId replay and conflict, deterministic source grouping, verified briefing baselines, fail-closed briefing capacity, exact lease behavior and recovery, deterministic claim/evidence/quality functions, byte-stable rendering, fully verified version publication, current-or-suspended commit, and target-first recovery across every materialization point, while its root stays empty until a complete runtime exists. Facade fixtures cover every public mapping, RequestId creation and preservation, subject binding, null-to-void projection, injected-client close delegation, its exact runtime/type export allowlists, built package-name calls, and absence of Node built-ins. MCP fixtures use the official in-memory client to check exact descriptors, every action mapping, mutation context, empty results, suspended presentation, parsed dual output, redacted errors, admission draining, five-second shutdown, and borrowed port ownership. Its built smoke uses the official stdio client against a real child for initialize, tools/list, all five calls, invalid input, exact JSON text projection, quiet bounded teardown, and the separate root/stdio export faces; the child injects a full test-only EngineClient and presenter and therefore proves no local backend, correction flow, or product e2e. The fail-closed TypeScript-AST boundary gate now encodes exactly `@distilly/engine → @distilly/protocol`, `distilly → @distilly/protocol`, and `@distilly/mcp → @distilly/protocol`, rejects Facade/MCP edges to Engine and MCP to Facade, and continues to catch manifest/source direction, relative-import bypasses, non-static module targets, and package cycles through `npm:`, `workspace:`, `link:`, directory `file:`, and bare directory aliases. The CI definition exercises Node 22.19 and 24 across Linux and macOS; the Python documentation, Agent Note, compile, unittest, and Ruff lanes remain in place for the legacy tree. The governed-diff predicate includes `packages/` and the root pnpm, TypeScript, ESLint, Vitest, Knip, and Prettier configuration surfaces while preserving the standing tests-only exemption for co-located Vitest files and snapshots.

This evidence proves the workspace, serializable Protocol contract, exact current package boundary, Engine fact primitives and package-internal Step 7 behavior, injected-client Facade/MCP projections, and real stdio transport conformance over a full fake, not the full lifecycle proposed here. Production runtime/MCP/CLI/setup composition, a `DISTILLY_ROOT` backend, Panel and review/correction/evolution services, the public `prompt()` / generated `SKILL.md` / plugin host-instruction snapshots, host workflows, fresh install, release automation, and eventual parity-backed retirement of Python governance remain unimplemented, so this Note remains proposed.
