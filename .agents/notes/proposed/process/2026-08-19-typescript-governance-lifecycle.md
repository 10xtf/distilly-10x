...
The implemented slice is a pinned `pnpm` workspace, the V3 §29.1 `@distilly/protocol` contract, and `@distilly/engine` through its package-internal Step 7 claim-commit slice. Its real stages cover formatting and lint, no-emit typecheck, fail-closed Vitest discovery, reported coverage, readable Protocol and packaged host-distill prompt snapshots, TypeScript build, package self-import, exports and type resolution, and unused or undeclared dependency hygiene. Protocol gates cover the 35-method schema registry, the exact five MCP descriptors, an explicit public-export allowlist, and the built entry. The second package adds a fail-closed TypeScript-AST gate for manifest/source dependency direction, relative-import bypasses, non-static module targets, and package cycles; it follows `npm:` and `workspace:` aliases plus local `link:`, directory `file:`, and bare directory specs back to the real internal target instead of trusting the dependency key. Engine fixtures exercise real facts, cross-process locks, atomic create/existing ingest, global RequestId replay and conflict, deterministic source grouping, verified briefing baselines, fail-closed briefing capacity, exact lease behavior and recovery, deterministic claim/evidence/quality functions, byte-stable rendering, fully verified version publication, current-or-suspended commit, and target-first recovery across every materialization point, while its root stays empty until a complete runtime exists. The built smoke imports `@distilly/engine` by package-name self-reference, reads the packaged prompt, and uses real child processes for lock, create, and commit contention so the export map, package contents, and cross-process transaction behavior are part of the check. The CI definition exercises Node 22.19 and 24 across Linux and macOS; the Python documentation, Agent Note, compile, unittest, and Ruff lanes remain in place for the legacy tree. The governed-diff predicate includes `packages/` and the root pnpm, TypeScript, ESLint, Vitest, Knip, and Prettier configuration surfaces while preserving the standing tests-only exemption for co-located Vitest files and snapshots.

This evidence proves the workspace, serializable Protocol contract, package boundary, Engine fact primitives, and the package-internal Step 7 ingest/source-grouping/briefing/lease/claim-commit/version-recovery behavior, not the full lifecycle proposed here. The public `prompt()` / generated `SKILL.md` / plugin host-instruction snapshots, complete public `EngineRuntime`, review/correction/evolution workflows, host workflows, fresh install, release automation, and eventual parity-backed retirement of Python governance remain unimplemented, so this Note remains proposed.
