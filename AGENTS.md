...
- The current committed TypeScript tree is the `pnpm` workspace, `@distilly/protocol`, `@distilly/engine` through its package-internal verified reads, review/rollback, and JSON Library slices, the injected-client `distilly` Facade and `@distilly/mcp` adapter, the capability-only `@distilly/bindings` package plus canonical skill/release assembly, and the injected-client `@distilly/panel` server/web leaf. Engine still exports no runtime, the stdio conformance child and Panel browser fixture inject full test fakes rather than a `DISTILLY_ROOT` backend, and both host entries stop at trusted capability preflight. No complete single-writer Engine service, SQLite authority, full binding, production MCP/CLI/setup/Panel composition, correction service, plugin runtime, or user `~/.distilly/` installation is shipped. Uncommitted Step 11a file-journal work is paused and is not shipped-state evidence.
...
These are the gates that exist today. The TypeScript suite currently proves the workspace, Protocol wire contract, exact current package dependency direction, the committed package-internal file-based Engine through ingest/source-grouping/briefing/lease/claim-commit/version-recovery/read/review/rollback/Library projection, injected-client Facade/MCP mappings, real stdio transport conformance over a full test fake, capability-only host preflight, canonical skill/plugin release bytes, and the injected Panel's real loopback HTTP plus Chromium behavior. The target SQLite authority, production single-writer runtime, correction, full binding, CLI/setup/Panel composition, and later release tiers remain in [design §27](docs/design/v3/27-testing-and-governance.md).
