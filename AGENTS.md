...
- The current committed TypeScript tree is the `pnpm` workspace, Protocol, Adapters source contracts plus deterministic TXT/Markdown/JSON/SRT/VTT parsers, the SQLite/WAL Engine authority and Preview reads including raw/file ingest and host-operation replay, explicit Engine/LocalRuntime Preview subpaths, injected Facade/MCP/Panel leaves, capability-only plus full Codex/Claude Code bindings, canonical skill assembly, and the private CLI Preview composition. The CLI now has a repo-local Codex-first built executable with `setup`, narrow `doctor`, `uninstall`, and plugin-owned `mcp --host`; it probes Codex's exact version before writing, parses an immutable release-bound net-capacity record verified through the real host surface, writes an ancestor-checked absolute launcher, serves exactly five tools, lazily launches the real review Panel, and removes bootstrap bytes without removing person data or separately installed person Skills. Runtime composes a caller-selected full binding for verified `Person` install/export/uninstall while missing or mismatched bindings fail closed; a fresh local Codex process can discover the resulting self-contained Skill. The Panel can promote, reject, and roll back against that same SQLite root; deferred deep Doctor is reported as unavailable rather than blocking review recovery. Claude Code's full binding remains implemented but is not enabled by this CLI: the public command rejects it and the lifecycle provider fails closed without matching real-host evidence. Stable package roots remain intentionally narrow, Adapters has no built-in provider, and deferred MethodMap keys fail visibly. No self-contained release archive, upgrade, cross-process attach, production deep doctor, or published user installation is shipped.
