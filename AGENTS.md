...
- The current TypeScript product tree is the `pnpm` workspace, the V3 §29.1 `@distilly/protocol` slice, and `@distilly/engine` through its package-internal Step 6 briefing-and-lease slice. Engine now has verified facts and locks, atomic create/existing text ingest, deterministic `source-groups-v1`, a verified version/claims baseline reader, a packaged and snapshotted host-distill prompt, fail-closed briefing capacity, state-authoritative `brief`/`renew`/`release` lease transactions with crash recovery, and the rebuildable SQLite queue v2 projection; its package root still exports no partial runtime. No complete `EngineRuntime`, commit/version writer, facade, MCP runtime, CLI, bindings, Panel, plugins, or user `~/.distilly/` installation is shipped.
...
These are the gates that exist today. The TypeScript suite currently proves the workspace, Protocol wire contract, package dependency direction, Engine fact primitives, and the internal ingest/source-grouping/briefing/lease/recovery/queue v2 slice; a complete public `EngineRuntime` and later release tiers remain in [design §27](docs/design/v3/27-testing-and-governance.md).
