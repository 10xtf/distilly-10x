...
- The current local TypeScript tree is the `pnpm` workspace, the V3 §29.1 `@distilly/protocol` slice, `@distilly/engine` through its package-internal Step 7 claim-commit slice, the Step 8 injected-client `distilly` Facade and `@distilly/mcp` adapter, and the Step 9 capability-only `@distilly/bindings` package plus canonical skill/release assembly. Engine still exports no partial runtime, the stdio conformance child injects a full test fake rather than a `DISTILLY_ROOT` backend, and both host entries stop at trusted capability preflight. No complete `EngineRuntime`, full binding, production MCP/CLI/setup composition, review or correction service, Panel, plugin runtime, or user `~/.distilly/` installation is shipped.
...
These are the gates that exist today. The TypeScript suite currently proves the workspace, Protocol wire contract, exact current package dependency direction, Engine fact primitives and internal ingest/source-grouping/briefing/lease/claim-commit/version-recovery slice, injected-client Facade/MCP mappings, real stdio transport conformance over a full test fake, capability-only host preflight, and canonical skill/plugin release bytes; a production local runtime, full binding, CLI/setup path, Panel, correction service, and later release tiers remain in [design §27](docs/design/v3/27-testing-and-governance.md).
