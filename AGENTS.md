...
- The current TypeScript product tree is the `pnpm` workspace, the V3 §29.1 `@distilly/protocol` slice, and `@distilly/engine` through its package-internal Step 7 claim-commit slice. Engine now has verified facts and locks, atomic create/existing text ingest, deterministic source grouping and briefing, durable leases, deterministic claim/evidence/quality functions, byte-stable profile rendering, immutable version publication, current-or-suspended state commit, and target-first transaction recovery; its SQLite queue remains a rebuildable projection and its package root still exports no partial runtime. No complete `EngineRuntime`, review or correction services, facade, MCP runtime, CLI, bindings, Panel, plugins, or user `~/.distilly/` installation is shipped.
...
These are the gates that exist today. The TypeScript suite currently proves the workspace, Protocol wire contract, package dependency direction, Engine fact primitives, and the internal ingest/source-grouping/briefing/lease/claim-commit/version-recovery slice; a complete public `EngineRuntime` and later release tiers remain in [design §27](docs/design/v3/27-testing-and-governance.md).
