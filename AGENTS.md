...
- The current local TypeScript tree is the `pnpm` workspace, the V3 §29.1 `@distilly/protocol` slice, `@distilly/engine` through its package-internal Step 7 claim-commit slice, and the Step 8 injected-client adapters: browser-safe `distilly` exports `Distilly` + `Person`, while `@distilly/mcp` exports the exact five transport-neutral handlers and a separate stdio transport entry. Engine still exports no partial runtime, and the stdio conformance child injects a full test fake rather than a `DISTILLY_ROOT` backend. No complete `EngineRuntime`, production MCP/CLI/setup composition, review or correction service, binding, Panel, plugin runtime, or user `~/.distilly/` installation is shipped.
...
These are the gates that exist today. The TypeScript suite currently proves the workspace, Protocol wire contract, exact current package dependency direction, Engine fact primitives and internal ingest/source-grouping/briefing/lease/claim-commit/version-recovery slice, injected-client Facade/MCP mappings, and real stdio transport conformance over a full test fake; a production local runtime, CLI/setup path, Panel, correction service, and later release tiers remain in [design §27](docs/design/v3/27-testing-and-governance.md).
