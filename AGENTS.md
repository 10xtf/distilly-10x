...
- The current committed TypeScript tree is the `pnpm` workspace, Protocol, Adapters source contracts plus deterministic TXT/Markdown/JSON/SRT/VTT parsers, the SQLite/WAL Engine authority and Preview reads including raw/file ingest and host-operation replay, explicit Engine/LocalRuntime Preview subpaths, injected Facade/MCP/Panel leaves, capability-only plus full Codex/Claude Code bindings, canonical skill assembly, and the private CLI Preview composition. The local `0.1.0-preview.1` Codex-first package is assembled from production output with no workspace source/dependency, test fake, symlink, sentinel, or Claude tree; setup verifies every package byte, copies it to `~/.distilly/runtime/0.1.0-preview.1/`, and writes an absolute launcher to that copy. The CLI exposes `setup`, narrow `doctor`, explicit `install <subject-id> --host codex`, `uninstall`, and plugin-owned `mcp --host codex`; it probes Codex's exact version before writing, parses immutable release-bound capacity evidence verified through the real host surface, serves exactly five tools, and lazily launches the real review Panel. Runtime composes the full Codex binding for verified `Person` install/export/uninstall while missing or mismatched bindings fail closed. Fresh Codex processes discover both the Plugin Skill and separately installed person Skills; removing the unpacked bootstrap does not break the product, and Plugin uninstall removes only verified owned runtime/plugin bytes while preserving SQLite and person Skills. The Panel can promote, reject, and roll back against that same SQLite root; deferred deep Doctor is reported as unavailable rather than blocking review recovery. Claude Code's full binding remains implemented but is not enabled by this package or CLI: the public command rejects it and the lifecycle provider fails closed without matching real-host evidence. Stable package roots remain intentionally narrow, Adapters has no built-in provider, and deferred MethodMap keys fail visibly. No upgrade, cross-process attach, production deep doctor, Claude packaged activation, or published user installation is shipped.
...
These are the gates that exist today. The TypeScript suite proves the workspace, Protocol wire contract, source-adapter contracts and deterministic TXT/Markdown/JSON/SRT/VTT parsers, exact package direction, SQLite create/text-and-file-ingest/distill/review/correction authority and crash behavior, Preview reads, host-operation replay/provenance, Engine/LocalRuntime, injected Facade/MCP/Panel behavior, verified preflight plus concrete Codex/Claude Code plugin and person-projection bindings, and canonical release bytes. The self-contained Codex package gate additionally proves setup from an unpacked path containing spaces and non-ASCII characters, digest-owned runtime copying, unpacked-bootstrap removal, official Plugin/MCP discovery, exactly five descriptors, create/ingest/brief/commit/get/prompt, Chromium-driven correction promotion, explicit person-Skill installation, close/reopen, fresh Codex Skill discovery, Plugin uninstall, and byte-identical preservation of SQLite and the person Skill. The Codex capacity fixture records real-host structured/text and briefing boundary observations. Claude Code remains fail closed pending the same evidence. Local-file tests prove raw-only fallback, RequestId replay before file reads, one canonical text per raw, digest-verified reopen, and no persisted absolute path. Remaining SQLite methods, cross-process runtime, production upgrade/deep doctor, Claude packaged activation, and later release tiers remain in [design §27](docs/design/v3/27-testing-and-governance.md).
