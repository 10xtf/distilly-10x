...
- The current committed TypeScript tree is the `pnpm` workspace, `@distilly/protocol`, `@distilly/engine` with package-private SQLite/WAL `subjects.create` and `materials.ingest` plus legacy-fixture coverage of its unmigrated verified-read/review/rollback/Library slices, the injected-client `distilly` Facade and `@distilly/mcp` adapter, the capability-only `@distilly/bindings` package plus canonical skill/release assembly, and the injected-client `@distilly/panel` server/web leaf. Engine still exports no runtime, the stdio conformance child and Panel browser fixture inject full test fakes rather than a `DISTILLY_ROOT` backend, and both host entries stop at trusted capability preflight. No complete single-writer Engine service, full SQLite method authority, full binding, production MCP/CLI/setup/Panel composition, correction service, plugin runtime, or user `~/.distilly/` installation is shipped.
...
These are the gates that exist today. The TypeScript suite currently proves the workspace, Protocol wire contract, exact current package dependency direction, package-private SQLite create/ingest transactions and real crash behavior, legacy-fixture source-grouping/briefing/lease/claim-commit/version-recovery/read/review/rollback/Library behavior pending migration, injected-client Facade/MCP mappings, real stdio transport conformance over a full test fake, capability-only host preflight, canonical skill/plugin release bytes, and the injected Panel's real loopback HTTP plus Chromium behavior. The remaining SQLite method migrations, production single-writer runtime, correction, full binding, CLI/setup/Panel composition, and later release tiers remain in [design §27](docs/design/v3/27-testing-and-governance.md).
