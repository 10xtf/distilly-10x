...
- The current TypeScript product tree is the `pnpm` workspace, the V3 §29.1 `@distilly/protocol` slice, and `@distilly/engine` through its internal Step 5 atomic-ingest slice. Engine now has verified facts and locks, atomic create/existing text ingest, root request idempotency and prepared-journal recovery, auto/now pending markers, and a rebuildable SQLite queue projection; its package root still exports no partial runtime. No complete EngineRuntime, briefing/commit, facade, MCP server, CLI, bindings, Panel, plugins, or user `~/.distilly/` installation is shipped.
...
These are the gates that exist today. The TypeScript suite currently proves the workspace, Protocol wire contract, package dependency direction, Engine fact primitives, and the internal atomic-ingest/journal-recovery/queue slice; a complete public EngineRuntime and later release tiers remain in [design §27](docs/design/v3/27-testing-and-governance.md).
