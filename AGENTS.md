...
- The current committed TypeScript tree is the `pnpm` workspace, Protocol, contract-only Adapters, the SQLite/WAL Engine authority and Preview reads, explicit Engine/LocalRuntime Preview subpaths, injected Facade/MCP/Panel leaves, capability-only bindings, canonical skill assembly, and the private `@distilly/cli/preview` outer composition. The CLI Preview binds separate host and direct-user clients to one real root, serves exactly five MCP tools, lazily opens the real Panel for correction review, and has a real built stdio/reopen smoke; its stable root remains empty. Adapters still has no built-in provider or MaterialParser. No executable, setup/doctor/uninstall lifecycle, cross-process attach, full binding, plugin runtime, or user `~/.distilly/` installation is shipped.
...
These are the gates that exist today. The TypeScript suite proves the workspace, Protocol wire contract, source-adapter contracts, package direction, SQLite create/ingest/distill/review/correction authority and crash behavior, Preview reads, Engine/LocalRuntime, injected Facade/MCP/Panel behavior, capability-only preflight, canonical release bytes, and the real CLI Preview graph through stdio, correction Panel, close, and process reopen. Remaining SQLite methods, cross-process runtime, full bindings, executable/setup lifecycle, and later release tiers remain in [design §27](docs/design/v3/27-testing-and-governance.md).
