...
The `codex/distilly-plugin` branch is the current local Developer Preview. It packages a real TypeScript/SQLite product path for Codex: local material intake, versioned Person Profiles, five MCP tools, correction review, and explicit profile Skill installation. It is intentionally separate from the default `dot-skill` release line.
