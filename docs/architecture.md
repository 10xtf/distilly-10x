...
- `packages/engine/` now provides the package-private SQLite/WAL create, ingest, pending, brief, renew, release, commit, active-review query, promote, reject, and rollback slice. Its exact seventeen-table canonical private schema v1 owns spaces, subjects, aliases and identity hints, current material membership, authoritative pending jobs and optional leases, immutable versions, version-scoped claims/evidence/material membership, independent version status and current/suspended pointers, stable operations, blob-backed operation-result references, and audit events. Immutable normalized bodies and complete canonical briefing templates live in the SHA-256 content-addressed blob store. Every mutation commits its structured effects in one short transaction and replays RequestIds exactly.
...
- SQLite is therefore current authority for create, ingest, pending, brief, renew, release, commit, active-review listing, promote, reject, and rollback. The remaining profile/material/version/Library reads and projections, correction, and other product methods still require feature-by-feature migration before a complete root-scoped Engine writer exists; every migration removes its replaced file path instead of adapting or dual-writing it.
...
internal engine:    packages/engine/src   → SQLite create/ingest/brief/lease/commit/review/rollback + legacy read/Library fixtures
