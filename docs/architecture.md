...
- `packages/engine/` now provides the first package-private SQLite/WAL business-method slice. A strict ten-table schema owns spaces, subjects, aliases and identity hints, current material membership, authoritative pending rows, stable operations, and audit events; immutable normalized bodies live in a SHA-256 content-addressed blob store. Standalone `subjects.create` and `materials.ingest(existing|create)` share one transaction-local identity primitive, commit every structured effect in one short transaction, hold one mutation-scoped blob access lease through commit or rollback, verify already referenced blobs without repairing missing content, replay RequestIds exactly, and expose no partial package-root API or full Engine runtime. Schema v1 has no versions authority and therefore requires both version pointers to remain NULL.
- An explicit `legacy-file-engine.test.fixture.ts` keeps the unmigrated package-internal Step 6 behavior executable without entering the SQLite composition. Those tests cover deterministic `source-groups-v1`, verified immutable version/claims baselines, the packaged host-distill prompt and evidence rules, briefing capacity, incremental briefing construction, and file-backed `brief`/`renew`/`release` lease transactions pending their own migration.
...
- The old `node:sqlite` queue v2 and checksummed JSON Library projection remain only behind the explicit legacy test fixture for unmigrated lease/read/review tests. Their file-owned pending/lease facts, dirty markers, writer-intent locks, and recovery behavior are still regression-tested but are not imported, read, or written by the new SQLite composition. The package root intentionally exports no partial Engine API.
- SQLite is therefore current authority for create/ingest only. Brief/lease, commit, verified reads, review/rollback, Library, and correction still require feature-by-feature migration before a complete root-scoped Engine writer exists; every migration removes its replaced file path instead of adapting or dual-writing it.
...
internal engine:    packages/engine/src   → SQLite create/ingest + test-only legacy fixture for unmigrated methods
