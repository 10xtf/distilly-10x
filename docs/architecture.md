...
- No complete Engine factory or `EngineRuntime`, commit/version writer and renderer, facade, MCP runtime, CLI, binding, Panel, plugin runtime, or user `~/.distilly/` installation exists yet. The internal Step 6 composition is testable implementation evidence, not a runnable user product.
...
- CI on `dot-skill`, `distilly`, and `main` reports documentation and Agent Note governance once, exercises the TypeScript workspace, Protocol contract, package-boundary gate, and Engine fact/ingest/briefing/lease/recovery/queue slices on Node 22.19 and 24 across Linux and macOS, and retains Python 3.9/3.11 compile and fail-closed unittest lanes plus Ruff while the legacy skill remains.
...
internal engine:    packages/engine/src   → verified facts + atomic ingest + incremental brief/lease/recovery + queue v2
