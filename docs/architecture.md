...
- An explicit `legacy-file-engine.test.fixture.ts` keeps the unmigrated package-internal read/review pipeline executable without entering the SQLite composition. Its supporting test-only files retain the former file-backed commit, immutable-version staging and semantic recovery solely as regression setup for read, review and rollback tests. Those writers, staging paths and recovery services cannot enter the production package and are not a compatibility fallback.
...
- The retired file-backed commit writer, immutable-version staging and recovery, review/rollback mutation authority, sibling queue, and business locks survive only through explicitly named test fixtures and are excluded from the production package. The still-unmigrated file-backed verified reads, review query, checksummed JSON Library projection, and their fact stores remain package-internal modules that are packaged but uncomposed. The live SQLite composition has no import or runtime path to either group and creates no queue database, version-staging directory, or dirty marker. The package root intentionally exports no partial Engine API.
- SQLite is therefore current authority for create, ingest, pending, brief, renew, release, and commit. Verified public reads/projections, review/rollback, Library, correction, and the remaining methods still require feature-by-feature migration before a complete root-scoped Engine writer exists; every migration removes its replaced file path instead of adapting or dual-writing it.
...
- CI on `dot-skill`, `distilly`, and `main` reports documentation and Agent Note governance once, exercises the TypeScript workspace, Protocol and source-adapter contracts, the package-boundary gate, Engine fact/ingest/briefing/lease/claim-commit/version-recovery/read/review/rollback/queue/Library slices, Facade/MCP/bindings/Panel tests, built stdio child conformance, real Chromium Panel behavior, and plugin release assembly on Node 22.19 and 24 across Linux and macOS, and retains Python 3.9/3.11 compile and fail-closed unittest lanes plus Ruff while the legacy skill remains.
...
internal engine:    packages/engine/src   → SQLite create/ingest/pending/brief/lease/commit + test-only legacy read/review fixtures
