...
- An explicit `legacy-file-engine.test.fixture.ts` keeps the unmigrated package-internal material/profile/version/Library read pipeline executable without entering the SQLite composition. Supporting test-only files retain the former file-backed commit, immutable-version staging, lease/commit recovery, and a copied legacy review query solely as regression setup for those reads. The former file review/rollback mutation service and its recovery branches are removed; none of these fixtures is a compatibility fallback or production-package entry.
- The still-unmigrated package-internal file read slice implements `library.list`, `materials.list/get`, `profiles.get/prompt/status`, and `versions.list/diff/lineage` over verified facts. Subject reads reconcile retained lease/commit journals, hold the subject lock through one complete fact snapshot, require every physical immutable version and lineage reference to have one consistent durable lifecycle, bind cursors to method and normalized filters, and fail closed for orphaned or contradictory versions; `raw_extract` remains unsupported without a verified raw reader.
- The retired file-backed commit writer, immutable-version staging and lease/commit recovery survive only through explicitly named test fixtures and are excluded from the production package. The still-unmigrated file-backed verified reads, checksummed JSON Library projection, their fact stores, and their read coordination remain package-internal modules that are packaged but uncomposed. A source-graph gate proves the live SQLite composition has no transitive dependency on that authority, locks, recovery, queue, or projection; it creates no queue database, version-staging directory, or dirty marker. The package root intentionally exports no partial Engine API.
- SQLite is therefore current authority for create, ingest, pending, brief, renew, release, commit, active-review listing, promote, reject, rollback, and correction. The remaining profile/material/version/Library reads and projections and other product methods still require feature-by-feature migration before a complete root-scoped Engine writer exists; every migration removes its replaced file path instead of adapting or dual-writing it.
...
- The current internal dependency edges are `@distilly/adapters → @distilly/protocol`, `@distilly/engine → @distilly/protocol`, `distilly → @distilly/protocol`, `@distilly/mcp → @distilly/protocol`, `@distilly/bindings → @distilly/protocol`, and `@distilly/panel → @distilly/protocol + @distilly/mcp`; the Panel web graph itself reaches only Protocol. Packages have no upward or mutual edge beyond that allowlist; negative fixtures cover bare, subpath, workspace, link, file, and directory aliases.
...
- No complete Engine factory or `EngineRuntime`, production MCP/CLI/setup/Panel composition, remaining evolution services, built-in source adapter, MaterialParser surface or implementation, user collection service, full binding, concrete injector/form renderer, plugin runtime, or user `~/.distilly/` installation exists yet. The package-internal Engine, source contracts, injected-client adapters and Panel, capability preflight, and source-only plugin assets are testable implementation evidence, not a runnable user product.
...
internal engine:    packages/engine/src   → SQLite create/ingest/brief/lease/commit/review/rollback/correction + legacy read/Library fixtures
