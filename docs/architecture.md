...
- The only new internal dependency edges are `distilly → @distilly/protocol` and `@distilly/mcp → @distilly/protocol`; both packages remain independent of `@distilly/engine`, and MCP remains independent of the facade. The boundary checker encodes these exact allowlists and negative fixtures.
...
- No complete Engine factory or `EngineRuntime`, production MCP/CLI/setup composition, review/correction/evolution service, binding, Panel, plugin runtime, or user `~/.distilly/` installation exists yet. The internal Step 7 composition and Step 8 injected-client adapters are testable implementation evidence, not a runnable user product.
...
- CI on `dot-skill`, `distilly`, and `main` reports documentation and Agent Note governance once, exercises the TypeScript workspace, Protocol contract, package-boundary gate, Engine fact/ingest/briefing/lease/claim-commit/version-recovery/queue slices, Facade/MCP adapter tests, and built stdio child conformance on Node 22.19 and 24 across Linux and macOS, and retains Python 3.9/3.11 compile and fail-closed unittest lanes plus Ruff while the legacy skill remains.
