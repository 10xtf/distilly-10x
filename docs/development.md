...
Python 3.9+ remains required for the published skill and repository governance. The TypeScript product requires Node `^22.19 || ^24`; Corepack reads the pinned package manager from `package.json`, and `pnpm install --frozen-lockfile` installs exactly the committed lock. The workspace currently contains Protocol; contract-only source-adapter and direct-user collection boundaries; package-private SQLite/WAL create, ingest, pending, brief, renew, release, claim commit, active-review query, promote, reject, rollback, correction, and Preview-required subject/Profile/material/version/lineage/Library reads over canonical private schema v1; the opt-in in-process `@distilly/engine/preview` and `@distilly/runtime/preview` composition; retained uncomposed file-read/Library regression modules plus test-only legacy mutation fixtures; the injected-client Facade/MCP adapter; capability-only bindings plus canonical skill/release assembly; and the injected-client Panel server/web leaf. Adapters has no built-in provider, MaterialParser surface or implementation, secret resolver, or collection service. Stable runtime roots remain empty; the Preview subpaths dispatch the real local-material methods and explicitly reject deferred keys. The stdio and Panel browser fixtures still use full test fakes, and the host entries perform preflight without installation or composition, so there is not yet a runnable installed user product ([design §27](design/v3/27-testing-and-governance.md)). Other methods and authenticated cross-process attach still need implementation over the one-writer SQLite authority.
...
| Host capability bindings and registry | `pnpm exec vitest run packages/bindings/src` |
...
Product code goes in the `packages/` workspaces named in [design §25](design/v3/25-package-and-source-tree.md): `@distilly/protocol`, `@distilly/engine`, `@distilly/runtime`, `@distilly/adapters`, `@distilly/bindings`, `@distilly/mcp`, `@distilly/panel`, `@distilly/cli`, and the `distilly` facade. The dependency direction is one-way: engine, bindings, adapters, facade, MCP, and Panel web depend on protocol; Panel server additionally depends on MCP's narrow ReviewPresenter type. The current Preview runtime composes Protocol plus Engine only; the production target later composes bindings and adapters there as their interfaces land. CLI composes runtime + MCP + Panel. A shared wire type belongs in protocol, not whichever package needed it first.

The workspace, its gates, Protocol, contract-only source-adapter and direct-user collection boundaries, package-private SQLite create/ingest/pending/brief/renew/release/commit/review/rollback/correction plus Preview verified reads, explicit in-process Preview Engine/LocalRuntime, injected-client adapters, capability-only bindings/canonical plugin source, and injected-client Panel are implemented locally. Deterministic evidence/claim/quality/rendering logic feeds the SQLite commit and correction transactions; verified active pointers, statuses, immutable versions, current material membership, events, and blobs feed the live SQLite reads. The replaced file commit writer, immutable-version staging, and lease/commit recovery remain only in explicit test fixtures and are excluded from the production package; the former file review/rollback mutation fixture and its recovery branches are removed. Correction uses the shared blob store and SQLite transaction without a correction journal, staging, or recovery path. The superseded file verified reads, JSON Library projection, their fact stores, and their read coordination remain package-internal, packaged, but uncomposed for regression coverage; a source-graph gate prevents the live SQLite composition from reaching them. The Facade and MCP packages remain injected-client adapters; the adapters package owns no concrete provider, MaterialParser surface or implementation, secret resolver, or collection service; Panel server/web consumes a borrowed complete EngineClient and never creates a runtime; bindings translate only injected trusted preflight evidence; and the plugin assembler exact-mirrors a recursive canonical skill and pins source release bytes. The current internal edges are adapters/engine/facade/MCP/bindings to Protocol, Preview runtime to Protocol plus Engine, and Panel to Protocol plus MCP, with the Panel web graph restricted to Protocol. A complete cross-process single-writer service, remaining SQLite method authority, built-in source collection, production MCP/CLI/setup/Panel composition, remaining evolution/Recall/raw services, full binding/injector/form renderer, and installable plugin runtime remain target design; do not infer them from the legacy fixtures, source-only templates, Protocol contracts, the stdio fake, or the Panel browser fake. `tools/` and `prompts/` continue to take defect fixes for the published skill only — V3 prompts live under `packages/engine/prompts/` ([design §29.1](design/v3/29-landing-and-evolution.md)).
