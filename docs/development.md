...
Python 3.9+ remains required for the published skill and repository governance. The TypeScript product requires Node `^22.19 || ^24`; Corepack reads the pinned package manager from `package.json`, and `pnpm install --frozen-lockfile` installs exactly the committed lock. The workspace currently contains the V3 §29.1 `@distilly/protocol` slice, `@distilly/engine` through its package-internal Step 7 claim-commit slice, and the Step 8 injected-client `distilly` Facade plus `@distilly/mcp` adapter and stdio transport. Engine still exposes no incomplete runtime, and the stdio conformance child uses a full test fake rather than a `DISTILLY_ROOT` backend, so there is not yet a runnable user product ([design §27](design/v3/27-testing-and-governance.md)).
...
| Engine facts, ingest, briefing/lease, claim commit, version recovery, and queue | `pnpm exec vitest run packages/engine/src` |
...
The workspace, its gates, the V3 §29.1 `@distilly/protocol` slice, `@distilly/engine` through package-internal Step 7, and the Step 8 injected-client `distilly` and `@distilly/mcp` packages are implemented locally. Engine layout, hashes, atomic facts and staging, concrete stores and locks, globally idempotent create/existing text ingest, deterministic `source-groups-v1`, verified briefing baselines, the packaged host-distill prompt, fail-closed capacity, state-authoritative lease transactions, deterministic claim commit and rendering, fully verified immutable versions, current/suspended publication, and rebuildable queue v2 recovery are current code. The Facade maps its browser-safe `Distilly` and `Person` surface to a supplied complete `EngineClient`; MCP maps exactly five Protocol tools and offers a separate stdio transport over supplied client/presenter ports. The only current internal edges are engine/facade/MCP to Protocol, and the boundary gate rejects Facade/MCP imports of Engine plus MCP imports of the Facade. The complete `EngineRuntime`, production MCP/CLI/setup composition, `DISTILLY_ROOT` backend, review/correction/evolution services, bindings, Panel, and plugin runtime remain target design; do not infer them from Protocol types, injected adapters, or the full fake used by the stdio smoke. `tools/` and `prompts/` continue to take defect fixes for the published skill only — V3 prompts live under `packages/engine/prompts/`, with no third tree invented to avoid the package cut ([design §29.1](design/v3/29-landing-and-evolution.md)).
