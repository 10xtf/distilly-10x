...
Python 3.9+ remains required for the published skill and repository governance. The TypeScript product requires Node `^22.19 || ^24`; Corepack reads the pinned package manager from `package.json`, and `pnpm install --frozen-lockfile` installs exactly the committed lock. The workspace currently contains the V3 §29.1 `@distilly/protocol` slice and `@distilly/engine` through its package-internal Step 5 atomic-ingest, prepared-journal recovery, and rebuildable queue slice. The package root still exposes no incomplete runtime, so there is not yet a runnable user product ([design §27](design/v3/27-testing-and-governance.md)).
...
| Engine facts and internal atomic ingest/recovery/queue | `pnpm exec vitest run packages/engine/src` |
...
The workspace, its gates, the V3 §29.1 `@distilly/protocol` slice, and `@distilly/engine` through internal Step 5 are implemented. Engine layout, hashes, atomic facts and staging, concrete stores and locks, globally idempotent create/existing text ingest, prepared-journal recovery, pending-state derivation, and the rebuildable queue projection are current code. The complete EngineRuntime, pending briefing/lease, commit/version pipeline, and every other package named above remain target design; do not create empty packages or infer later behavior from protocol types. `tools/` and `prompts/` continue to take defect fixes for the published skill only — no V3 behavior and no third tree invented to avoid the package cut ([design §29.1](design/v3/29-landing-and-evolution.md)).
