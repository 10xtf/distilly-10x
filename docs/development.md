...
Python 3.9+ remains required for the published skill and repository governance. The TypeScript product requires Node `^22.19 || ^24`; Corepack reads the pinned package manager from `package.json`, and `pnpm install --frozen-lockfile` installs exactly the committed lock. The workspace currently contains the V3 §29.1 Protocol, package-internal Engine through Step 10 verified reads/review/rollback/Library projection, the Step 8 injected-client Facade/MCP adapter, the Step 9 capability-only bindings plus canonical skill/release assembly, and the Step 10 injected-client Panel server/web leaf. Engine still exposes no incomplete runtime, the stdio and Panel browser fixtures use full test fakes, and the host entries perform preflight without installation or composition, so there is not yet a runnable user product ([design §27](design/v3/27-testing-and-governance.md)).
...
The workspace, its gates, the V3 §29.1 Protocol, Engine through package-internal Step 10, Step 8 injected-client adapters, Step 9 capability-only bindings/canonical plugin source, and Step 10 injected-client Panel are implemented locally. Engine layout, hashes, facts, ingest, source grouping, briefing, lease, deterministic claim commit/rendering, immutable versions, current/suspended publication, verified read snapshots, review/rollback journals, queue recovery, and the checksummed JSON Library projection are current code. The Facade and MCP packages remain injected-client adapters; Panel server/web consumes a borrowed complete EngineClient and never creates a runtime; bindings translate only injected trusted preflight evidence; and the plugin assembler exact-mirrors a recursive canonical skill and pins source release bytes. The current internal edges are engine/facade/MCP/bindings to Protocol plus Panel to Protocol and MCP, with the Panel web graph restricted to Protocol. A complete `EngineRuntime`, production MCP/CLI/setup/Panel composition, `DISTILLY_ROOT` backend, correction/evolution services, full binding/injector/form renderer, and installable plugin runtime remain target design; do not infer them from source-only templates, Protocol contracts, the stdio fake, or the Panel browser fake. `tools/` and `prompts/` continue to take defect fixes for the published skill only — V3 prompts live under `packages/engine/prompts/`, with no third tree invented to avoid the package cut ([design §29.1](design/v3/29-landing-and-evolution.md)).
