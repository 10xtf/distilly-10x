...
| Engine SQLite create/ingest plus legacy-fixture briefing/lease, claim commit, verified reads, review/rollback, queue, and Library behavior | `pnpm exec vitest run packages/engine/src` |
...
The workspace, its gates, Protocol, package-private SQLite create/ingest, injected-client adapters, capability-only bindings/canonical plugin source, and injected-client Panel are implemented locally. The old file facts, source grouping, briefing, lease, deterministic claim commit/rendering, immutable versions, verified reads, review/rollback journals, queue recovery, and JSON Library projection remain only as code exercised through an explicit test fixture until their owner migrations replace them. The Facade and MCP packages remain injected-client adapters; Panel server/web consumes a borrowed complete EngineClient and never creates a runtime; bindings translate only injected trusted preflight evidence; and the plugin assembler exact-mirrors a recursive canonical skill and pins source release bytes. The current internal edges are engine/facade/MCP/bindings to Protocol plus Panel to Protocol and MCP, with the Panel web graph restricted to Protocol. A complete single-writer Engine, remaining SQLite method authority, production MCP/CLI/setup/Panel composition, correction/evolution/Recall/raw services, full binding/injector/form renderer, and installable plugin runtime remain target design; do not infer them from the legacy fixture, source-only templates, Protocol contracts, the stdio fake, or the Panel browser fake. `tools/` and `prompts/` continue to take defect fixes for the published skill only — V3 prompts live under `packages/engine/prompts/` ([design §29.1](design/v3/29-landing-and-evolution.md)).
