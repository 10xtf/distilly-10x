...
Python 3.9+ remains required for the published skill and repository governance. The TypeScript product requires Node `^22.19 || ^24`; Corepack reads the pinned package manager from `package.json`, and `pnpm install --frozen-lockfile` installs exactly the committed lock. The workspace currently contains Protocol; contract-only source-adapter and direct-user collection boundaries; package-private SQLite/WAL create, ingest, pending, brief, renew, release, claim commit, active-review query, promote, reject, rollback, and correction over canonical private schema v1; uncomposed package-internal verified-read and Library modules plus test-only legacy mutation fixtures; the injected-client Facade/MCP adapter; capability-only bindings plus canonical skill/release assembly; and the injected-client Panel server/web leaf. Adapters has no built-in provider, MaterialParser surface or implementation, secret resolver, or collection service. Engine still exposes no runtime, the stdio and Panel browser fixtures use full test fakes, and the host entries perform preflight without installation or composition, so there is not yet a runnable user product ([design §27](design/v3/27-testing-and-governance.md)). The remaining profile/material/version/Library reads and projections and other methods still need migration into the one-writer SQLite authority.
...
| Engine SQLite create/ingest/pending/brief/lease/commit/review/rollback/correction plus legacy-fixture verified reads, queue, and Library behavior | `pnpm exec vitest run packages/engine/src` |
...
The workspace, its gates, Protocol, contract-only source-adapter and direct-user collection boundaries, package-private SQLite create/ingest/pending/brief/renew/release/commit/review/rollback/correction, injected-client adapters, capability-only bindings/canonical plugin source, and injected-client Panel are implemented locally. Deterministic evidence/claim/quality/rendering logic feeds the SQLite commit and correction transactions; verified active pointers, statuses, immutable versions, and current material membership feed the SQLite review transactions. The replaced file commit writer, immutable-version staging, and lease/commit recovery remain only in explicit test fixtures and are excluded from the production package; the former file review/rollback mutation fixture and its recovery branches are removed. Correction uses the shared blob store and SQLite transaction without a correction journal, staging, or recovery path. The unmigrated file verified reads, JSON Library projection, their fact stores, and their read coordination remain package-internal, packaged, but uncomposed; a source-graph gate prevents the live SQLite composition from reaching them. The Facade and MCP packages remain injected-client adapters; the adapters package owns no concrete provider, MaterialParser surface or implementation, secret resolver, or collection service; Panel server/web consumes a borrowed complete EngineClient and never creates a runtime; bindings translate only injected trusted preflight evidence; and the plugin assembler exact-mirrors a recursive canonical skill and pins source release bytes. The current internal edges are adapters/engine/facade/MCP/bindings to Protocol plus Panel to Protocol and MCP, with the Panel web graph restricted to Protocol. A complete single-writer Engine, remaining SQLite method and read/projection authority, built-in source collection, production MCP/CLI/setup/Panel composition, remaining evolution/Recall/raw services, full binding/injector/form renderer, and installable plugin runtime remain target design; do not infer them from the legacy fixtures, source-only templates, Protocol contracts, the stdio fake, or the Panel browser fake. `tools/` and `prompts/` continue to take defect fixes for the published skill only — V3 prompts live under `packages/engine/prompts/` ([design §29.1](design/v3/29-landing-and-evolution.md)).
