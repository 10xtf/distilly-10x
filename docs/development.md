...
Python 3.9+ remains required for the published skill and repository governance. The TypeScript product requires Node `^22.19 || ^24`; Corepack reads the pinned package manager from `package.json`, and `pnpm install --frozen-lockfile` installs exactly the committed lock. The workspace contains the Protocol, SQLite Preview authority and verified reads, opt-in Engine/Runtime, injected Facade/MCP/Panel leaves, capability-only bindings, canonical skill assembly, and an explicit private `@distilly/cli/preview` outer graph over a real local root. The CLI graph binds separate host and direct-user clients, serves exactly five MCP tools, lazily opens the real Panel for review, and reopens persisted Profiles; the stable CLI root remains empty. There is still no executable, host setup/doctor/uninstall, full binding, authenticated cross-process attach, or installed product ([design §27](design/v3/27-testing-and-governance.md)).
...
The workspace, its gates, Protocol, SQLite create/ingest/distill/review/correction authority and Preview reads, explicit in-process Engine/LocalRuntime, injected Facade/MCP/Panel leaves, capability bindings, and canonical plugin source are implemented locally. The private CLI Preview package now composes those leaves over one real root; lower packages remain independently injected and the CLI alone has the reviewed upward dependency edges. The superseded file paths remain isolated regression surfaces. A complete cross-process service, local file parsers, remaining methods, executable/setup lifecycle, full bindings/injectors, host profile installation, and installable plugin runtime remain target work; do not infer them from the explicit Preview subpath or source-only templates.
