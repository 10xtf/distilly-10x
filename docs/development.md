...
Python 3.9+ remains required for the published skill and repository governance. The TypeScript product requires Node `^22.19 || ^24`; Corepack reads the pinned package manager from `package.json`, and `pnpm install --frozen-lockfile` installs exactly the committed lock. The workspace currently contains the V3 §29.1 `@distilly/protocol` slice and `@distilly/engine` through its package-internal Step 6 incremental-briefing and lease slice, including deterministic source grouping, verified baseline reads, packaged prompt loading, fail-closed capacity, lease recovery, and queue v2. The package root still exposes no incomplete runtime, so there is not yet a runnable user product ([design §27](design/v3/27-testing-and-governance.md)).
...
| Engine facts, ingest, briefing/lease, recovery, and queue | `pnpm exec vitest run packages/engine/src` |
...
The workspace, its gates, the V3 §29.1 `@distilly/protocol` slice, and `@distilly/engine` through package-internal Step 6 are implemented. Engine layout, hashes, atomic facts and staging, concrete stores and locks, globally idempotent create/existing text ingest, deterministic `source-groups-v1`, verified version/claims baseline reads, the packaged host-distill prompt, fail-closed briefing capacity, incremental briefing construction, state-authoritative `brief`/`renew`/`release` transactions and recovery, and the rebuildable queue v2 projection are current code. The complete `EngineRuntime`, commit/version writer and renderer, and every other package named above remain target design; do not create empty packages or infer later behavior from Protocol types. `tools/` and `prompts/` continue to take defect fixes for the published skill only — V3 prompts live under `packages/engine/prompts/`, with no third tree invented to avoid the package cut ([design §29.1](design/v3/29-landing-and-evolution.md)).
