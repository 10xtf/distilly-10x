...
| Engine facts, ingest, briefing/lease, claim commit, verified reads, review/rollback recovery, queue, and Library projection | `pnpm exec vitest run packages/engine/src` |
...
The workspace, its gates, Protocol, the package-internal Engine through review/rollback/Library, injected-client adapters, capability-only bindings/canonical plugin source, and injected-client Panel are implemented locally. Engine layout, hashes, file facts, ingest, source grouping, briefing, lease, deterministic claim commit/rendering, immutable versions, current/suspended publication, verified read snapshots, review/rollback journals, queue recovery, and the checksummed JSON Library projection are current code pending replacement. The Facade and MCP packages remain injected-client adapters; Panel server/web consumes a borrowed complete EngineClient and never creates a runtime; bindings translate only injected trusted preflight evidence; and the plugin assembler exact-mirrors a recursive canonical skill and pins source release bytes. The current internal edges are engine/facade/MCP/bindings to Protocol plus Panel to Protocol and MCP, with the Panel web graph restricted to Protocol. A single-writer SQLite authority, complete runtime, production MCP/CLI/setup/Panel composition, correction/evolution/Recall/raw services, full binding/injector/form renderer, and installable plugin runtime remain target design; do not infer them from uncommitted Step 11a files, source-only templates, Protocol contracts, the stdio fake, or the Panel browser fake. `tools/` and `prompts/` continue to take defect fixes for the published skill only — V3 prompts live under `packages/engine/prompts/` ([design §29.1](design/v3/29-landing-and-evolution.md)).
