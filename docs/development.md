...
Python 3.9+ remains required for the published skill and repository governance. The TypeScript product requires Node `^22.19 || ^24`; Corepack reads the pinned package manager from `package.json`, and `pnpm install --frozen-lockfile` installs exactly the committed lock. The workspace currently contains Protocol; package-private SQLite/WAL create, ingest, pending, brief, renew, release, and claim commit over canonical private schema v1; uncomposed package-internal verified-read, review-query, and Library modules plus test-only legacy mutation fixtures; the injected-client Facade/MCP adapter; capability-only bindings plus canonical skill/release assembly; and the injected-client Panel server/web leaf. Engine still exposes no runtime, the stdio and Panel browser fixtures use full test fakes, and the host entries perform preflight without installation or composition, so there is not yet a runnable user product ([design §27](design/v3/27-testing-and-governance.md)). Reads/projections, review/rollback, correction, and the remaining methods still need migration into the one-writer SQLite authority.
...
The workspace, its gates, Protocol, package-private SQLite create/ingest/pending/brief/renew/release/commit, injected-client adapters, capability-only bindings/canonical plugin source, and injected-client Panel are implemented locally. Deterministic evidence/claim/quality/rendering logic feeds the SQLite commit transaction. The replaced file commit writer, immutable-version staging and recovery, review/rollback mutation authority, sibling queue, and business locks remain only in explicit test fixtures and are excluded from the production package. The unmigrated file verified reads, review query, JSON Library projection, and their fact stores remain package-internal, packaged, but uncomposed; the live SQLite composition imports neither group. The Facade and MCP packages remain injected-client adapters; Panel server/web consumes a borrowed complete EngineClient and never creates a runtime; bindings translate only injected trusted preflight evidence; and the plugin assembler exact-mirrors a recursive canonical skill and pins source release bytes. The current internal edges are engine/facade/MCP/bindings to Protocol plus Panel to Protocol and MCP, with the Panel web graph restricted to Protocol. A complete single-writer Engine, remaining SQLite method authority, production MCP/CLI/setup/Panel composition, correction/evolution/Recall/raw services, full binding/injector/form renderer, and installable plugin runtime remain target design; do not infer them from the legacy fixtures, source-only templates, Protocol contracts, the stdio fake, or the Panel browser fake. `tools/` and `prompts/` continue to take defect fixes for the published skill only — V3 prompts live under `packages/engine/prompts/` ([design §29.1](design/v3/29-landing-and-evolution.md)).
