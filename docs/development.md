...
Python 3.9+ remains required for the published skill and repository governance. The TypeScript product requires Node `^22.19 || ^24`; Corepack reads the pinned package manager from `package.json`, and `pnpm install --frozen-lockfile` installs exactly the committed lock. The workspace currently contains the V3 §29.1 `@distilly/protocol` slice and the internal `@distilly/engine` fact foundation. It does not yet contain a complete Engine runtime or runnable product ([design §27](design/v3/27-testing-and-governance.md)).
...
| Engine fact foundation | `pnpm exec vitest run packages/engine/src` |
...
The workspace, its gates, the V3 §29.1 `@distilly/protocol` slice, and `@distilly/engine`'s internal fact foundation are implemented. Engine layout, hashes, atomic fact files, concrete stores, and writer locks are current code; create/ingest, journals/recovery, queueing, and the complete EngineRuntime remain target design. Every other package named above is also still target design; do not create empty packages or infer later behavior from protocol types. `tools/` and `prompts/` continue to take defect fixes for the published skill only — no V3 behavior and no third tree invented to avoid the package cut ([design §29.1](design/v3/29-landing-and-evolution.md)).
