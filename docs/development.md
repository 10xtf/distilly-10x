...
| Engine SQLite create/ingest/pending/brief/lease/commit plus legacy-fixture verified reads, review/rollback, queue, and Library behavior | `pnpm exec vitest run packages/engine/src` |
...
The workspace, its gates, Protocol, contract-only source-adapter and direct-user collection boundaries, package-private SQLite create/ingest/pending/brief/renew/release/commit, injected-client adapters, capability-only bindings/canonical plugin source, and injected-client Panel are implemented locally. Deterministic evidence/claim/quality/rendering logic feeds the SQLite commit transaction. The replaced file commit writer, immutable-version staging and recovery, review/rollback mutation authority, sibling queue, and business locks remain only in explicit test fixtures and are excluded from the production package. The unmigrated file verified reads, review query, JSON Library projection, and their fact stores remain package-internal, packaged, but uncomposed; the live SQLite composition imports neither group. The Facade and MCP packages remain injected-client adapters; the adapters package owns no concrete provider, MaterialParser surface or implementation, secret resolver, or collection service; Panel server/web consumes a borrowed complete EngineClient and never creates a runtime; bindings translate only injected trusted preflight evidence; and the plugin assembler exact-mirrors a recursive canonical skill and pins source release bytes. The current internal edges are adapters/engine/facade/MCP/bindings to Protocol plus Panel to Protocol and MCP, with the Panel web graph restricted to Protocol. A complete single-writer Engine, remaining SQLite method authority, built-in source collection, production MCP/CLI/setup/Panel composition, correction/evolution/Recall/raw services, full binding/injector/form renderer, and installable plugin runtime remain target design; do not infer them from the legacy fixtures, source-only templates, Protocol contracts, the stdio fake, or the Panel browser fake. `tools/` and `prompts/` continue to take defect fixes for the published skill only — V3 prompts live under `packages/engine/prompts/` ([design §29.1](design/v3/29-landing-and-evolution.md)).
