...
| Real Preview Facade/MCP/Panel composition, person projections, and reopen | `pnpm exec vitest run packages/cli/src/preview.test.ts`; set `DISTILLY_VERIFY_CODEX_DISCOVERY` to a real Codex executable for the opt-in fresh-process discovery check; after build, `pnpm --filter @distilly/cli run smoke:built` |
