...
Python 3.9+ remains required for the published skill and repository governance. The TypeScript product requires Node `^22.19 || ^24`; Corepack reads the pinned package manager from `package.json`, and `pnpm install --frozen-lockfile` installs exactly the committed lock. The workspace currently contains Protocol; package-private SQLite/WAL create, ingest, pending, brief, renew, and release over canonical private schema v1; test-only legacy coverage for the unmigrated claim/version methods; the injected-client Facade/MCP adapter; capability-only bindings plus canonical skill/release assembly; and the injected-client Panel server/web leaf. Engine still exposes no runtime, the stdio and Panel browser fixtures use full test fakes, and the host entries perform preflight without installation or composition, so there is not yet a runnable user product ([design §27](design/v3/27-testing-and-governance.md)). Commit, reads/projections, review/rollback, correction, and the remaining methods still need migration into the one-writer SQLite authority.
...
| Engine SQLite create/ingest/pending/brief/lease plus legacy-fixture claim commit, verified reads, review/rollback, queue, and Library behavior | `pnpm exec vitest run packages/engine/src` |
