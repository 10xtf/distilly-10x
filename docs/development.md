...
Python 3.9+ remains required for the published skill and repository governance. The TypeScript product requires Node `^22.19 || ^24`; Corepack reads the pinned package manager from `package.json`, and `pnpm install --frozen-lockfile` installs exactly the committed lock. The workspace currently contains the V3 §29.1 Protocol, package-internal Engine through Step 7, the Step 8 injected-client Facade/MCP adapter, and the Step 9 capability-only bindings plus canonical skill/release assembly. Engine still exposes no incomplete runtime, the stdio conformance child uses a full test fake, and the host entries perform preflight without installation or composition, so there is not yet a runnable user product ([design §27](design/v3/27-testing-and-governance.md)).
...
The workspace, its gates, the V3 §29.1 Protocol, Engine through package-internal Step 7, Step 8 injected-client adapters, and Step 9 capability-only bindings/canonical plugin source are implemented locally. Engine layout, hashes, facts, ingest, source grouping, briefing, lease, deterministic claim commit/rendering, immutable versions, current/suspended publication, and queue recovery are current code. The Facade and MCP packages remain injected-client adapters; bindings translate only injected trusted preflight evidence, while the plugin assembler exact-mirrors a recursive canonical skill and pins source release bytes. The only current internal edges are engine/facade/MCP/bindings to Protocol. A complete `EngineRuntime`, production MCP/CLI/setup composition, `DISTILLY_ROOT` backend, review/correction/evolution services, full binding/injector/form renderer, Panel, and installable plugin runtime remain target design; do not infer them from source-only templates, Protocol contracts, or the stdio fake. `tools/` and `prompts/` continue to take defect fixes for the published skill only — V3 prompts live under `packages/engine/prompts/`, with no third tree invented to avoid the package cut ([design §29.1](design/v3/29-landing-and-evolution.md)).
